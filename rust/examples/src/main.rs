...
fn airdrop_sol_to_payer(payer: &Keypair, sols: u64) {
    ...
    let signature = rpc_client
        .request_airdrop_with_blockhash(&payer.pubkey(), sols * 1_000_000_000, &blockhash)
        .unwrap();
    // Wait for airdrop to be confirmed
    rpc_client
        .confirm_transaction_with_spinner(&signature, &blockhash, rpc_client.commitment())
        .unwrap();
    println!("Airdropped {} SOL to {}", sols, payer.pubkey());
}

fn get_fulfilled_randomness(requestor: &VrfRequestor, seed: &Pubkey) -> Randomness {
    let mut err_count = 0;
    loop {
        if err_count > 5 {
...
fn verify_randomness_loop(requestor: &VrfRequestor, seed: &Pubkey, randomness: &Randomness) {
    let mut attempts = 0;
    while attempts < 5 {
